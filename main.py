...
class LoginScreen(Screen):
    def login_user(self, username, password):
        ...
        conn.close()
        if result:
            Snackbar(text=f"Welcome {username}!").open()
...
class SignupScreen(Screen):
    def signup_user(self, username, password):
        if username and password:
            ...
            Snackbar(text="Enter username & password").open()
...
class RegisterScreen(Screen):
    def get_gps_location(self):
        if gps:
            try:
                ...
                Snackbar(text="GPS not supported").open()
        else:
            Snackbar(text="GPS not available").open()
    ...
    def register_donor(self, name, bg, phone, lat, lon):
        if name and bg and phone:
            ...
            conn.commit()
            conn.close()
            Snackbar(text=f"Donor {name} registered!").open()
        else:
...
class SearchScreen(Screen):
    def search_donor(self, bg):
        ...
        if results:
            ...
            Snackbar(text="No donors found").open()
    ...
    def export_csv(self):
        ...
        if data:
            ...
            Snackbar(text="Exported donors.csv").open()
        else:
            Snackbar(text="No donors to export").open()
    ...
    def call_donor(self, phone):
        if call:
            try:
                ...
                Snackbar(text=f"Call failed: {e}").open()
        else:
